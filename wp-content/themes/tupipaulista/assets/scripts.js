...
function scrollTop() {
    ...
    var scrolling = function scrolling() {
        ...
        var startTop = (0, _jquery2.default)('#header').height();

        interval = true;

        if (scrollTop > startTop) body.addClass('scrolling');else body.removeClass('scrolling');

        if (scrollTop < currentScrollTop && scrollTop > startTop) {
    ...
    (0, _jquery2.default)(window).bind('scroll', scrolling);
...
(0, _jquery2.default)(document).ready(function () {
    (0, _jquery2.default)('[data-carousel=banner]').slick({
...
document.addEventListener("DOMContentLoaded", function () {
    var menuPrimary = document.querySelector('#menu');
    var buttons = (0, _from2.default)(document.querySelectorAll('[aria-controls]'));

    if (buttons.length) {
        buttons.forEach(function (item) {
            return item.addEventListener('click', toggleMenu.bind(undefined));
        });
    }

    function toggleMenu(e) {
        var menuId = e.currentTarget.getAttribute('aria-controls');
        var menu = document.querySelector('#' + menuId);

        if (menu) {
            menuActived(menuId, menu);
        }
    }

    function menuActived(menuId, menu) {
        if (menu.classList.contains(menuId + '--actived')) {
            menu.classList.remove(menuId + '--actived');
            menuFixedBody(menuId, false);
        } else {
            menu.classList.add(menuId + '--actived');
            menuFixedBody(menuId, true);
        }
    }

    function menuFixedBody(menuId) {
        var flag = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

        var body = document.querySelector('body');

        if (flag) body.classList.add(menuId + '--fixed');else body.classList.remove(menuId + '--fixed');
    }

    menuPrimary.addEventListener('click', function (e) {
        if (e.target.classList.contains('menu--menu') || e.target.classList.contains('menu__container')) {
            menuActived('menu', menuPrimary);
        }
    });
});
